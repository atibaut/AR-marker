...
public class PropManager {
	...
	private Transform3D chgT3d; // holds current change to the posn, rot, or
								// scale

	private String filename; // of loaded object
	private double xRot, yRot, zRot; // total of rotation angles carried out
	private ArrayList<Integer> rotInfo; // stores the sequence of rotation
										// numbers
	 ...
	 * 
	 * The loaded object has 4 transform groups above it -- objBoundsTG is for
	 * adjusting the object's bounded sphere so it is centered at (0,0,0) and
	 * has unit radius.
	 * 
	 * The other TGs are for doing separate moves, rotations, and scaling of the
	 * object. moveTG-->rotTG-->scaleTG-->objBoundsTG-->object
	 */
	{
		...
		try {
			if (ext.equals("obj")) { // the file is a wavefront model
				...
				of.setFlags(ObjectFile.RESIZE | ObjectFile.TRIANGULATE
						| ObjectFile.STRIPIFY);
				s = of.load("models/" + fnm);
			} else { // use Portfolio loader for other models
				ModelLoader modelLoader = new ModelLoader();
				s = modelLoader.load("models/" + fnm); // handles many types of
														// file
			}
		} catch (Exception e) {
	...
	// file contains a 3ds model
	{
		...
		if (ext.equals("3ds")) { // the file is a 3ds model
			...
			rotTrans.rotX(-Math.PI / 2.0); // 3ds models are often on their
											// face; fix that
	...
	/*
	 * Obtain coords info from the coordinates file for fnm. The coords file has
	 * the format: <3D object fnm> [-p px py pz] [-r sequence of numbers] [-s
	 * scale]
	 */
	{
		...
		try {
			...
			while ((line = br.readLine()) != null) {
				...
				if (ch == 'p')
					...
					System.out.println(coordFile + ": did not recognise line: "
							+ line);
			}
			br.close();
	...
	// then apply it to the loaded object
	{
		...
		while (tokens.hasMoreTokens()) {
			...
			try {
				...
				System.out
						.println("Incorrect format for position data in coords file");
				break;
			}
		}
		if (count != 3)
	...
	// and apply it to the loaded object
	{
		...
		for (int i = 0; i < token.length(); i++) {
			try {
				...
				System.out
						.println("Incorrect format for rotation data in coords file");
				break;
			}
			if (rotNum == 1) // positive x-axis rotation
				...
				System.out
						.println("Did not recognise the rotation info in the coords file");
		}
	...
	// and apply it to the loaded object
	{
		...
		try {
			...
			System.out
					.println("Incorrect format for scale data in coords file");
			startScale = 1.0;
		}
	...
	/*
	 * These methods are called when applying the coords file information *and*
	 * when user commands sent from the GUI are being processed.
	 */
	...
	// create a coords file for this object
	{
		...
		try {
			...
			out.println("-p " + df.format(currLoc.x) + " "
	...
	{
		System.out.println(id + " x: " + df.format(t.x) + ", " + id + " y: "
				+ df.format(t.y) + ", " + id + " z: " + df.format(t.z));
	} // end of printTuple()
